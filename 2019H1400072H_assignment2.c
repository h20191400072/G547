...
static int usbdev_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
	...
	if((id->idProduct == USB_KINGSTON_PID))
	{
		printk(KERN_INFO "KINGSTON USB drive detected.\n");
	}
	else if((id->idProduct ==  HDD_PID))
	{
		printk(KERN_INFO "WD HDD drive detected.\n");
	}
	else{
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Rusha Mondal <h20191400072@hyderabad.bits-pilani.ac.in> ");
MODULE DESCRIPTION(" USB DEV DETECT");
